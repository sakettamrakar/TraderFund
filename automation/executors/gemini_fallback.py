...
def ask(prompt: str) -> str:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            # Always use temp file + stdin pipe approach for reliability on Windows.
            # Writing to a temp file and piping it avoids shell escaping issues with
            # special characters in prompts (quotes, newlines, braces in JSON).
            ...
            try:
                with open(prompt_file, "r", encoding="utf-8") as pf:
                    result = subprocess.run(
                        ...
                        capture_output=True,
                        text=True,
                        shell=False,  # No shell on Windows for reliable stdin piping
            ...
            last_error = f"Gemini CLI not found at '{GEMINI_PATH}'. Is it installed?"
            raise RuntimeError(last_error)
